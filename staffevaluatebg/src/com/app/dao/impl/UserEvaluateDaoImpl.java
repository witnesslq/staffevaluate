...
public class UserEvaluateDaoImpl extends HibernateDaoSupport implements UserEvaluateDao {
	...
	public void updateReturn(String ids) {
		...
		for(int i=0; i<idarr.length; i++){
			getHibernateTemplate().bulkUpdate("update UserEvaluate ue set ue.isReturn = '1' where ue.id = ?", Integer.valueOf(idarr[i]));
		}
	}
	...
	public void updateValid(String ids) {
		String idarr[] = ids.split(",");
		for(int i=0; i<idarr.length; i++){
			getHibernateTemplate().bulkUpdate("update UserEvaluate ue set ue.isValid = '1' where ue.id = ?", Integer.valueOf(idarr[i]));
		}
	}
